...
// main task reducer
const task_reducer = createReducer(
  initialStates,
  ...
  on(deleteTask, (state: any, action) => {
    ...
    const tasksAfterDeleting = state.tasks.filter(
      (singleTask: any) => singleTask.id !== action.deleteId
    );
    return {
      ...state,
  ...
  on(deleteCategorySearch, (state: any, action) => {
    ...
    const checkRemaining = state.tasks.some(
      (task: any) => task.category.toLowerCase() === category.toLowerCase()
    );
    let newCategories = [];
    if (!checkRemaining) {
      ...
      if (task.id === taskId) {
        return {
          ...
          comments: task.comments.filter(
            (comment: any) => comment.id !== deleteCommentId
          ),
        };
      } else {
        return task;
  ...
  on(filterTasks, (state: any, action) => {
    ...
    if (searchTerm.toLowerCase() === 'all') {
      ...
      const filteredTasks: any = existingTasks.filter(
        (task: any) =>
  ...
  on(uploadDoneTasks, (state: any) => {
    const newDoneTasks = state.tasks.filter(
      (singleTask: any) => singleTask.inprogress === true
    );
    return {
      ...state,
